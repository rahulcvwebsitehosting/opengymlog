...
function ExerciseDetail({ ex, close }) {
  ...
  const userNote = st.userNotes?.[ex.id]
  return <>
    ...
    {userNote && <div className="exnote user-note"><Icon name="note" style={{ marginRight: 6 }} />{userNote}</div>}
    ...
    <div className="row" style={{ gap: 8, marginTop: 8 }}>
      <Button icon="pencil" style={{ flex: 1 }} onClick={() => { close(); noteEditorSheet(ex) }}>{userNote ? t('Edit note') : t('Add note')}</Button>
      {ex.custom && <Button variant="danger" icon="trash" style={{ flex: 1 }} onClick={() => deleteCustomEx(ex, close)}>{t('Delete')}</Button>}
    </div>
...
function NoteEditor({ ex, close }) {
  const st = useStore(s => s.S)
  const [note, setNote] = useState(st.userNotes?.[ex.id] || '')
  const save = () => {
    const trimmed = note.trim()
    update(s => {
      if (!s.userNotes) s.userNotes = {}
      if (trimmed) s.userNotes[ex.id] = trimmed.slice(0, 1000)
      else delete s.userNotes[ex.id]
    })
    close()
    toast(trimmed ? t('Note saved') : t('Note removed'))
  }
  return <>
    <h3>{note ? t('Edit note') : t('Add note')}</h3>
    <div className="muted small" style={{ marginBottom: 12 }}>{t('Save setup details like seat height, pin position, grip cues — shown on the exercise card and during workouts.')}</div>
    <textarea className="input" rows={5} maxLength={1000} placeholder={t('e.g. Seat height: 4\nPin position: 2\nGrip: second position')}
      value={note} onChange={e => setNote(e.target.value)} />
    <div style={{ height: 14 }} />
    <Button variant="primary" onClick={save}>{t('Save')}</Button>
    {note && <><div style={{ height: 8 }} /><Button variant="danger" onClick={() => { setNote(''); save() }} style={{ flex: 1 }}>{t('Remove note')}</Button></>}
  </>
}
export const noteEditorSheet = ex => ui().openSheet(close => <NoteEditor ex={ex} close={close} />)
