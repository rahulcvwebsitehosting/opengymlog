...
(async () => {
  ...
  try {
    ...
    if (cached) d = JSON.parse(cached)
      ...
      const r = await fetch('https://api.github.com/repos/rahulcvwebsitehosting/opengymlog')
      if (!r.ok) return
      d = await r.json()
...
;(async () => {
  ...
  try {
    ...
    if (cached) rel = JSON.parse(cached)
      ...
      const r = await fetch('https://api.github.com/repos/rahulcvwebsitehosting/opengymlog/releases?per_page=100')
      if (!r.ok) return
