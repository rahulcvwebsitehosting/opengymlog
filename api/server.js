/* opengym-api — username/password + passkey (WebAuthn) auth + per-user state storage for openGym
   No framework, JSON-file storage, signed session cookies + JWT for mobile.               */
...
// JWT secret for mobile app tokens (separate from cookie secret)
const JWT_SECRET_FILE = path.join(DATA, 'jwt_secret');
if (!fs.existsSync(JWT_SECRET_FILE)) fs.writeFileSync(JWT_SECRET_FILE, crypto.randomBytes(32).toString('hex'), { mode: 0o600 });
...
function requireAuth(req, res) {
  ...
  if (user.disabled) { json(res, 403, { error: 'account disabled' }); return null; }
  return user;
}
...
const routes = {
  ...
  'GET /api/config': async (req, res) => json(res, 200, { invite_only: INVITE_ONLY, auth_methods: ['password', 'passkey'] }),
  ...
  'POST /api/auth/register': async (req, res) => {
    ...
    if (!username || !name || !password) return json(res, 400, { error: 'username, name and password required' });
    if (username.length < 3) return json(res, 400, { error: 'username must be at least 3 characters' });
    if (password.length < 8) return json(res, 400, { error: 'password must be at least 8 characters' });
    if (!/^[a-z0-9_-]+$/.test(username)) return json(res, 400, { error: 'username can only contain lowercase letters, numbers, underscore and hyphen' });
    if (db.users.find(u => u.username === username)) return json(res, 409, { error: 'username already taken' });
    ...
    const user = { id: uid, username, name, passwordHash, created: new Date().toISOString() };
    if (invite) { user.invitedBy = invite.code; invite.usedBy = user.id; invite.usedAt = user.created; }
    db.users.push(user);
    saveDb();

    const token = makeJWT(user);
    json(res, 200, { user: { id: user.id, name: user.name, username: user.username, admin: isAdmin(user) }, token }, { 'Set-Cookie': sessionCookie(user) });
  },
  ...
  'GET /api/admin/users': async (req, res) => {
    ...
    const users = db.users.map(u => {
      ...
      return {
        id: u.id, name: u.name, username: u.username || null, created: u.created || null,
